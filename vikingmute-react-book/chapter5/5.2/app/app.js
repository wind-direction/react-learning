...
const source = '<div class="operate"><p>文章列表:总数 {{posts.length}}</p>{{#if posts}}<ul>{{#each post}}<li>{{this.id}} --- {{this.title}}</li>{{/each}}</ul>{{/if}}<p> 用户信息：是否登录：{{user.isLogin}}</p>{{#if user.isLogin}}用户邮箱：{{user.userData.email}} 用户名：{{user.userData.name}}{{/if}}</div>';
...
function posts(state = initialPostsState, action) {
  switch (action.type) {
    case CREATE_POST: return [...state, action.data];
    case DELETE_POST: return state.filter(post => post.id !== action.id);
